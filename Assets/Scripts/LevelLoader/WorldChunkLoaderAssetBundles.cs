...
public class WorldChunkLoaderAssetBundles : MonoBehaviour
{
    ...
    IEnumerator LoadWorldObject(WorldChunk chunk, WorldObjectData worldObjectData, Transform parent)
    {
        ...
        if (worldObjectData.hasModel)
        {
            ...
            AssetBundleRequest subMeshesRequest = worldAssetBundle.LoadAssetWithSubAssetsAsync<Mesh>(worldObjectData.model);
            yield return new WaitWhile(() => subMeshesRequest.isDone == false);
            Mesh[] subMeshes = subMeshesRequest.allAssets as Mesh[];
